...
import type { ChatMsg } from "./new-case/types";
import { getNow } from "./new-case/types";
import { findNewCaseEntry, suggestedPrompts, EXTRACTED_PATIENT_DATA } from "./new-case/agent-entries";
import { ThinkingIndicator, UserBubble, AgentBubble, NextBestActionCard } from "./new-case/chat-components";
import { EHRConsentCard } from "./new-case/ehr-consent-card";
import { EHRRedirectOverlay } from "./new-case/ehr-redirect-overlay";
import { EHRAgentPanel } from "./new-case/ehr-agent-panel";
import { DocumentUploadZone, DocumentCaptureZone } from "./new-case/document-zones";
...
export const NewCasePage = forwardRef<NewCasePageHandle, { onBack: () => void }>(
  function NewCasePage({ onBack }, ref) {
    ...
    return (
      ...
      <div className="flex-1 min-w-0 flex flex-col h-full overflow-hidden bg-white border-r border-[#e5e5e5]">
        {/* Header */}
            ...
            {showEmptyState && (
              ...
              >
                <div className="flex flex-col items-center gap-3">
                  <div className="w-14 h-14 bg-[#1F425F] rounded-2xl flex items-center justify-center shadow-lg">
                    <Sparkles size={24} className="text-white" />
                  </div>
                  <p
                    className="font-['Ubuntu_Sans',sans-serif] text-[22px] leading-[30px] text-[#1b2124]"
                    style={{ fontWeight: 700 }}
                  >
                    Create a New Case
                  </p>
                  <p
                    className="font-['Ubuntu_Sans',sans-serif] text-[14px] leading-[22px] text-[#4d595e] text-center max-w-[420px]"
                ...
                <div className="grid grid-cols-2 gap-3 w-full">
                  {suggestedPrompts.map((sp, i) => (
                    <motion.button
                      ...
                      className="flex items-center gap-3 bg-white border border-[#e5e5e5] rounded-xl px-4 py-3.5 text-left cursor-pointer hover:border-[#1F425F]/30 hover:shadow-sm transition-all group"
                    >
                      <div className="w-8 h-8 bg-[#f7fafc] rounded-lg flex items-center justify-center shrink-0 group-hover:bg-[#eef4f8] transition-colors">
                        {sp.icon}
                      </div>
                      <span
