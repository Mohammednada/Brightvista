...
export function DocumentUploadZone({ onComplete }: { onComplete: () => void }) {
  ...
  return (
    <motion.div
      ...
      className="w-full max-w-[85%] rounded-xl border border-[#e5e5e5] bg-white overflow-hidden shadow-sm"
    >
      {phase === "idle" && (
        <div
        ...
        >
          ...
          <div className="flex flex-col items-center gap-1">
            <span className="font-['Ubuntu_Sans',sans-serif] text-[13px] text-[#1b2124]" style={{ fontWeight: 600 }}>
              Click to upload or drag & drop
            </span>
            <span className="font-['Ubuntu_Sans',sans-serif] text-[11px] text-[#6b7c93]">
              PDF, JPG, PNG, TIFF -- Max 25MB
            </span>
          </div>
          <div className="flex items-center gap-4 mt-1">
            {["Referral Letter", "Lab Results", "Insurance Card"].map(label => (
              <span key={label} className="font-['Ubuntu_Sans',sans-serif] text-[10px] text-[#3385f0] bg-[#f0f7ff] px-2 py-0.5 rounded-full">
                {label}
              </span>
            ))}
          </div>
        ...
        <div className="p-5 flex flex-col gap-3">
          <div className="flex items-center gap-3 bg-[#f9fafb] rounded-lg px-3 py-2.5 border border-[#e5e8ec]">
            ...
            <div className="flex-1 min-w-0">
              <span className="font-['Ubuntu_Sans',sans-serif] text-[12px] text-[#1b2124] block truncate" style={{ fontWeight: 500 }}>
                Patient_Referral_Thompson_Margaret.pdf
              </span>
              <span className="font-['Ubuntu_Sans',sans-serif] text-[10px] text-[#6b7c93]">2.4 MB</span>
            </div>
          ...
          <span className="font-['Ubuntu_Sans',sans-serif] text-[11px] text-[#6b7c93] text-center">
      ...
      {phase === "processing" && (
        <div className="p-5 flex flex-col gap-3">
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="font-['Ubuntu_Sans',sans-serif] text-[12px] text-[#1b2124]" style={{ fontWeight: 500 }}>
              Patient_Referral_Thompson_Margaret.pdf
            </span>
          </div>
          <div className="flex flex-col gap-1.5">
            {uploadOcrSteps.map((step, i) => (
              ...
              >
                ...
                <span
                  className={`font-['Ubuntu_Sans',sans-serif] text-[11px] ${i <= ocrStep ? "text-[#4d595e]" : "text-[#c0c8ce]"}`}
                  style={{ fontWeight: i === ocrStep ? 500 : 400 }}
                >
                  {step}
                </span>
              </motion.div>
      ...
      {phase === "done" && (
        <div className="p-5 flex flex-col items-center gap-2">
          ...
          <span className="font-['Ubuntu_Sans',sans-serif] text-[13px] text-[#166534]" style={{ fontWeight: 600 }}>
            Document processed successfully
          </span>
          <span className="font-['Ubuntu_Sans',sans-serif] text-[11px] text-[#4ade80]">
    ...
    >
      {phase === "viewfinder" && (
        <div className="flex flex-col">
          <div className="bg-[#0a0a0f] h-[200px] relative flex items-center justify-center">
            ...
            <span className="absolute bottom-3 font-['Ubuntu_Sans',sans-serif] text-[10px] text-white/60">
              Position document within the frame
            </span>
          </div>
          <button
            onClick={handleCapture}
            className="m-3 flex items-center justify-center gap-2 bg-[#1F425F] text-white rounded-lg py-2.5 cursor-pointer hover:bg-[#2d5a7a] transition-colors"
          >
            <Camera size={16} />
            <span className="font-['Ubuntu_Sans',sans-serif] text-[13px]" style={{ fontWeight: 500 }}>
              Capture Document
            </span>
          </button>
        </div>
      ...
      {phase === "captured" && (
        <div className="p-5 flex flex-col items-center gap-3">
          ...
          <span className="font-['Ubuntu_Sans',sans-serif] text-[12px] text-[#6b7c93]">
